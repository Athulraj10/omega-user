...
function* handleFetchWishlist() {
   try {
      ...
      yield put(actions.fetchWishlistSuccess(response.data || response))
   } catch (error: any) {
...
function* handleAddWishlist(action: any) {
   try {
      ...
      yield put(actions.addToWishlistSuccess(response.data || response))
   } catch (error: any) {
      yield put(
         actions.addToWishlistFailure(error.message || "Failed to add item to wishlist")
      )
   }
}
...
function* handleRemoveWishlist(action: any) {
   try {
      ...
      yield put(actions.removeFromWishlistSuccess(response.data || response))
   } catch (error: any) {
      yield put(
         actions.removeFromWishlistFailure(
